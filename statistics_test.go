...
func TestUpdateOneMeasurement(t *testing.T) {
	...
	fake.Update(Meta{"testcase": "sth", "elapsed": 8 * time.Millisecond, "timestamp": time.Now()})
...
func TestUpdateMultipleMeasurements(t *testing.T) {
	...
	fake.Update(Meta{"testcase": "sth", "elapsed": 8 * time.Millisecond, "timestamp": time.Now()})
	fake.Update(Meta{"testcase": "sth", "elapsed": 10 * time.Millisecond, "timestamp": time.Now()})
	fake.Update(Meta{"testcase": "sth", "elapsed": 2 * time.Millisecond, "timestamp": time.Now()})
	close(fake.measurements)
	<-done
	if v, ok := fake.stats["sth"]; ok {
...
func TestReport(t *testing.T) {
	...
	insert := func(name string) {
		fake.Update(Meta{"testcase": name, "elapsed": 8 * time.Millisecond, "timestamp": time.Now()})
		fake.Update(Meta{"testcase": name, "elapsed": 10 * time.Millisecond, "timestamp": time.Now()})
		fake.Update(Meta{"testcase": name, "elapsed": 2 * time.Millisecond, "timestamp": time.Now()})
	}
	insert("tc2")
	insert("tc1")
	insert("tc3")
